...
import { deleteFortuneRound } from "@/lib/actions/game";
...
const ListComponent = () => {
  const fortuneRounds = useFortuneRounds();
  const queryClient = useQueryClient();

  if (fortuneRounds.isLoading) return null;

  return (
    <div className="absolute flex flex-col w-full max-w-[20rem] p-4 bg-white rounded-lg left-10 top-10">
      <h3 className="text-lg font-medium">Fortune Rounds</h3>
      <ul className="">
        {fortuneRounds.data?.map((round, idx) => {
          return (
            <div className="flex w-full gap-4" key={round.id}>
              ...
              <Button
                variant={"ghost"}
                onClick={async () => {
                  const res = await deleteFortuneRound({ id: round.id });
                  if (res) {
